...
export const originByFullNameFields: INodeProperties[] = [
	{
...
async function postReceiveFunction(
	...
	return responseData.map((nameData) => {
		...
		const countries = (nameData.countriesOriginTop as string[]) || [];
		countries.forEach((country, index) => {
			json[`countryOrigin${index === 0 ? '' : index + 1}`] = country;
		});
...
export const originByFullNameDescription = {
